...
class Display(object):
    def __init__(self, state, animation_speed, name_agent):
        ...
        self._floor_height = round(HEIGHT_FLOORS / self._state.num_floors)  # To animate people. Total size is 600px
    ...
    def iteraction(self):
        for i in range(self._floor_height):
            ...
            self._canvas.move(self._elevator, 0, -self._state.cars[0].direction)  # animate the lift moving
            for person in self._state.people:
                if person.in_car:
                    self._canvas.move(person.animation, 0,
                                -self._state.cars[0].direction)  # animate the people moving
        time.sleep(self._animation_speed)

    def finish(self):
        longest_wait_time = max(self._state.wait_times)
        average_wait_time = sum(self._state.wait_times) / self._state.num_people
        ...
        print(self._state.num_floors, 'floor building with', self._state.num_people, 'people')
